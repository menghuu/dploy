...
def test_stow_with_existing_file_conflicts(source_a, source_c, dest):
    ...
    assert (error.ConflictsWithExistingFile(subcmd=SUBCMD,
                                            source=source_file,
...
def test_stow_with_source_conflicts(source_a, source_c, dest):
    ...
    assert (error.ConflictsWithAnotherSource(subcmd=SUBCMD,
...
def test_stow_with_file_as_source(file_a, dest):
    ...
    assert (error.NoSuchDirectory(subcmd=SUBCMD, file=file_a).msg
            in str(e.value))
...
def test_stow_with_file_as_dest(source_a, file_a):
    ...
    assert (error.NoSuchDirectoryToSubcmdInto(subcmd=SUBCMD, file=file_a).msg
            in str(e.value))
...
def test_stow_with_same_directory_used_as_source_and_dest(source_a):
    ...
    assert (error.SourceIsSameAsDest(subcmd=SUBCMD, file=source_a).msg
            in str(e.value))
...
def test_stow_with_same_simple_directory_used_as_source_and_dest(source_only_files):
    ...
    assert (error.SourceIsSameAsDest(subcmd=SUBCMD, file=source_only_files).msg
            in str(e.value))
...
def test_stow_with_source_dir_with_no_executue_permissions(source_a, source_c, dest):
    ...
    with pytest.raises(PermissionError) as e:
        dploy.stow([source_a, source_c], dest)
    assert (error.InsufficientPermissionsToSubcmdFrom(subcmd=SUBCMD, file=source_dir).msg
            in str(e.value))
...
def test_stow_unfolding_with_write_only_source_file(source_a, source_b, dest):
    ...
    with pytest.raises(PermissionError) as e:
        dploy.stow([source_a, source_b], dest)
    assert (error.InsufficientPermissionsToSubcmdFrom(subcmd=SUBCMD, file=source_file).msg
            in str(e.value))
